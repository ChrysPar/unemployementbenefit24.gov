$("document").ready(function () {
 ...
 function skipToEnd(message) {
    ...
    const error =
      currentLanguage === "greek"
        ? "Λυπούμαστε αλλά δεν δικαιούστε το επίδομα μακροχρονίως ανέργων!"
        : "We are sorry but you are not eligible to receive the long-term unemployment benefit.";
...
$("#startBtn").click(function () {
 ...
 getEvidencesById(1);
    ...
    if (
      allAnswers[4] === "1" ||
      (allAnswers[4] === "2" || allAnswers[4] === "4" )
    ) {
      getEvidencesById(1);
getEvidencesById(2);
getEvidencesById(3);
      currentLanguage === "greek"
        ? setResult("Είστε δικαιούχος!")
        : setResult("You are eligible for the long-term unemployment benefit!");
    }
  ...
  function submitForm() {
    ...
    currentLanguage === "greek"
      ? $(".question-container").append(
          "<br /><br /><h5 class='answer'>Τα δικαιολογητικά που πρέπει που απαιτούνται για να λάβετε το επίδομα είναι τα εξής:</h5><br />"
        )
      : $(".question-container").append(
          "<br /><br /><h5 class='answer'>The documents you need to provide in order to receive the long-term unemployment benefit are the following:</h5><br />"
        );
  ...
  $("#nextQuestion").click(function () {
    if ($(".govgr-radios__input").is(":checked")) {
      ...
      if (currentQuestion === 0 && selectedRadioButtonIndex === 2) {
        currentQuestion = -1;
  currentLanguage === "greek" ? skipToEnd("Λυπούμαστε αλλά δεν δικαιούστε το επίδομα μακροχρονίως ανέργων. Πρέπει να είστε τουλάχιστον 12 μήνες άνεργος!.") : skipToEnd("We are sorry, but you are not eligible for the long-term unemployment benefit. You must have been unemployed for at least 12 months!");
      } else if (currentQuestion === 1 && selectedRadioButtonIndex === 2) {
        currentQuestion = -1;
       currentLanguage === "greek" ? skipToEnd("Δυστυχώς, δεν είναι δυνατή η υποβολή αίτησης μετά την παρέλευση της δίμηνης προθεσμίας.") : skipToEnd("Unfortunately, it is not possible to submit an application after the two-month deadline has passed.");
      } else if (currentQuestion === 2 && selectedRadioButtonIndex === 2) {
        currentQuestion = -1;
   currentLanguage === "greek" ? skipToEnd("Δυστυχώς, δεν είναι δυνατή η υποβολή αίτησης καθώς πρέπει να είστε μέλος της ΔΥΠΑ") : skipToEnd("Unfortunately, it is not possible to submit an application as you must be a member of DYPA.");
      }  else if (currentQuestion === 3 && selectedRadioButtonIndex === 1) {
        currentQuestion = -1;
currentLanguage === "greek" ? skipToEnd("Δυστυχώς, δεν είναι δυνατή η υποβολή αίτησης καθώς πρέπει να έχετε συμπληρώσει τα 20 έτη.") : skipToEnd("Unfortunately, it is not possible to submit an application as you must have completed 20 years."); }
else if (currentQuestion === 3 && selectedRadioButtonIndex === 5) {
        currentQuestion = -1;
currentLanguage === "greek" ? skipToEnd("Δυστυχώς, δεν είναι δυνατή η υποβολή αίτησης καθώς πρέπει να μην ξεπερνάτε τα 66 έτη.") : skipToEnd("Unfortunately, it is not possible to submit an application as you must be under 66 years of age."); }
else if (currentQuestion === 4 && selectedRadioButtonIndex === 3) {
        currentQuestion = -1;
currentLanguage === "greek" ? skipToEnd("Δυστυχώς, δεν είναι δυνατή η υποβολή αίτησης καθώς δεν πρέπει να υπερβεί το ετήσιο οικογενειακό εισόδημα τις 10.000€.") : skipToEnd("Unfortunately, it is not possible to submit an application as the annual household income must not exceed €10,000"); }
else if (currentQuestion === 4 && selectedRadioButtonIndex === 5) {
        currentQuestion = -1;
currentLanguage === "greek" ? skipToEnd("Δυστυχώς, δεν είναι δυνατή η υποβολή αίτησης καθώς δεν πρέπει να υπερβεί το ετήσιο εκτιμώμενο οικογενειακό εισόδημα.") : skipToEnd("Unfortunately, it is not possible to submit an application as the annual estimated household income must not be exceeded."); }
else {
